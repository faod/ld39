...
Game::Game() : want_menu_(false), want_reload_(false)
{
	...
	win  = std::unique_ptr<ALLEGRO_SAMPLE, al_sample_deleter>(al_load_sample("data/win.ogg"));
...
void Game::load_game(std::string map_name)
{
    ...
    player_ = std::make_unique<Cyclist>(competitors_, level_->tracks[2].get_16px_percentage());
    add(*player_);
    add(*drawmap_object);
...
void Game::game_won()
{
	...
	al_play_sample(win.get(), 0.5, 0., 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
}
